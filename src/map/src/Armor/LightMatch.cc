...
void LightMatch::setEnemyColor(bool _enemy_blue, int _thresh_low, int filter){
...
void LightMatch::readAndConvert(cv::Mat& dst, std::vector<double>& pts){
	...
	if (cts.size() > 0) {
		...
		std::vector<std::vector<cv::Point> > max_ct = {ctref};
		cv::fillPoly(mask, max_ct, cv::Scalar(255));			// 最大的轮廓，内部填充，之后膨胀，成为最后的mask
